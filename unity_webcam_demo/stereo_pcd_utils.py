...
def get_model (args):
    ...
    if args.model == 'psmnet':
        print("::INFO:: Loading PSMNet model..")
        model = predict_img_psmnet.load_model(args.psmnet_model, args.psm_weights, args.no_cuda, args.seed, args.maxdisp)

    elif args.model == 'hitnet':
        print("::INFO:: Loading HITNet model..")
        model = predict_img_hitnet.load_model(args.hitnet_model_type, args.hitnet_weights)

    elif args.model == 'hsmnet':
...
def run_dispartiy (image_left, image_right, model, args):
    ...
    disp_L = None
    if args.model == 'psmnet':
        img_left, img_right = preprocess_image_psmnet(image_left, image_right)
        disp_L = predict_img_psmnet.test(img_left, img_right, model, is_cuda = (not args.no_cuda), model_type= args.psmnet_model)

    elif args.model == 'hitnet':
        disp_L = model(image_left, image_right)
